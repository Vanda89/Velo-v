...
var googleMaps = {
  ...
  init: function () {
    ...
    googleMaps.map = new google.maps.Map(
      document.getElementById("stations-map"),
      {
        center: {
          lat: 45.758,
          lng: 4.85,
  ...
  stations: function (stationsList) {
    ...
    for (var i = 0; i < googleMaps.stationsList.length; i++) {
      ...
      typeMarker =
        (googleMaps.stationsList[i].available_bikes * 100) /
        (googleMaps.stationsList[i].available_bike_stands +
          googleMaps.stationsList[i].available_bikes);
      ...
      if (
        googleMaps.stationsList[i].status !== "OPEN" ||
        (googleMaps.stationsList[i].available_bikes === 0 &&
          googleMaps.stationsList[i].available_bike_stands === 0)
      ) {
      ...
      var contentInfoWindow =
        ...
        (markers[i].status === "OPEN" ? "OUVERTE" : "FERMÉE") +
        "</p>" +
        "</div>";
      ...
      markers[i].addListener("click", function () {
        ...
        googleMaps.generateStationInfos(
          this.name,
          this.address,
          this.status,
  ...
  generateStationInfos: function (name, address, status, bikeStands, bikes) {
    ...
    let table =
      '<table id="stations-booked-infos"><caption>Détails de la station</caption>';
    table += '<tr id="name"><th>Nom : </th><td>' + name + "</td></tr>";
    table +=
      '<tr id="address"><th>Adresse : </th><td>' + address + "</td></tr>";
    table += '<tr id="status"><th>Statut : </th><td>' + status + "</td></tr>";
    table +=
      '<tr id="bikesStands"><th><i class="fab fa-product-hunt"></i></th><td>' +
      bikeStands +
      "</td></tr>";
    table +=
      '<tr id="bikes"><th><i class="fas fa-bicycle"></i></th><td>' +
      bikes +
      "</td></tr>";
    table += "</table>";
