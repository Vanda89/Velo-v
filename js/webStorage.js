var webStorage = {
  ...
  generateFooter: function () {
    ...
    let cancel = $('<button>').attr({
      ...
      type: "button",
      name: "cancel"
    }).text('Annuler la réservation').hide();
    let confirm = $('<button>').attr({
      id: "yes",
      type: "button",
      name: "yes"
    }).text('Oui').hide();
    let invalidate = $('<button>').attr({
      id: "no",
      type: "button",
      name: "no"
    }).text('Non').hide();
  ...
  startCountdown: function() {
    webStorage.count = setInterval(function() {
      ...
      var timeDifference = webStorage.currentTime - Number(sessionStorage.getItem('reservationTime'));
      ...
      var text = 'Vous avez réservé un vélo à la station ' + sessionStorage.getItem('station') + ' pour ';
      if (minutes > 0) {
      ...
      if (minutes <= 0 && secondes <= 0 || sessionStorage.getItem('station') === undefined) {
        clearInterval(webStorage.count);
        sessionStorage.clear();
        $('#infosCountdown').text('Votre réservation a expirée, effectuée s\'en une autre pour pouvoir prendre un vélo !');
        $('#cancel').hide();
      }
    }, 100);
